...
def get_palette(img):
    kmn = KMeans(n_clusters = 8, n_init = 10)
    kmn.fit(img_unique)
    palette = kmn.cluster_centers_.round().astype(np.uint8)
    palette_img = Image.fromarray(palette.reshape(-1, 1, 3))
    return palette_img.resize((32, 8 * 32), Image.Resampling.NEAREST)

source_image, matrix, img_unique, img_shape = load_image('image.jpg')
source_image.show()
palette = get_palette(img_unique)
palette.show()
